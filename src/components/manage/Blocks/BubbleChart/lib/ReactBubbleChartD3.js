...
export default class ReactBubbleChartD3 {
  ...
  update(el, props) {
    ...
    const nodes = this.bubble
      .nodes(data.length ? { children: data } : data)
    ...
    circles
      ...
      .style('opacity', 1)
      .style('fill', (d) =>
        d.selected ? this.selectedColor : color(d.colorValue),
      );
    ...
    labels
      ...
      .style('opacity', 1)
      .style('color', (d) =>
        d.selected ? this.selectedTextColor : textColor(d.colorValue),
      )
      .attr('class', (d) => {
        var size;
